...
public abstract class AbstractSpreadsheet
{
    ...
    private void Visit(string start, string name, ISet<string> visited, LinkedList<string> changed)
    {
        ...
        {
            // If we ever visit the cell we started with then the graph is circular
            if (n.Equals(start))
            {
                throw new CircularException();
            }
            // If we haven't visited a cell then visit it by checking its direct dependents
            else if (!visited.Contains(n))
            {
                Visit(start, n, visited, changed);
            }
        }
        // If we visited all the dependents of this cell without exception thrown
        // then this cell will be changed when the "name" cell change
